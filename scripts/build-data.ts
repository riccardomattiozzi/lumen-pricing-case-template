...
// ---------------------------------------------------------------------------
const rawSeasonalityAndWeather = loadCsv("seasonality_and_weather.csv").map(
  (r) => ({
    month: num(r.month),
    rawSeasonalityIndex: num(r.seasonality_index_100_avg),
    avgTempCelsius: num(r.avg_temp_germany_celsius),
  })
);
// The source column is labelled "100 avg", but its unweighted monthly mean
// is 101.67. Scale the processed index to a true 100-average without
// changing the source data or the seasonal shape the source describes.
const rawSeasonalityMean =
  rawSeasonalityAndWeather.reduce((sum, row) => sum + row.rawSeasonalityIndex, 0) /
  rawSeasonalityAndWeather.length;
const seasonalityAndWeather = rawSeasonalityAndWeather.map((row) => ({
  month: row.month,
  seasonalityIndex: round((row.rawSeasonalityIndex / rawSeasonalityMean) * 100, 4),
  avgTempCelsius: row.avgTempCelsius,
}));
...
**${round(rawSeasonalityMean, 2)}**, rather than the 100 stated in the column name.
The raw CSV is retained unchanged. Before writing \`seasonalityAndWeather.json\`, the
pipeline scales every index by \`100 / ${round(rawSeasonalityMean, 4)}\`; this preserves
