...
We built a simulator, not a single answer, because the honest answer to "what price should LUMEN launch at in Germany" depends on how much you weight Elena's runway against Jonas's brand ambitions — and we didn't think that call was ours to make silently inside a spreadsheet. Move any slider (price, channel mix, marketing budget, launch month, or the explicitly uncertain number in this model — how much of the German market LUMEN could realistically win in year one) and every number on the page recomputes live from the supplied case data and named assumptions: no hidden Excel tab, no rounding "for simplicity."
